...
export default function EditCourse({ course, categories, tools }: EditCourseProps) {
    ...
    const [showStrikethroughPrice, setShowStrikethroughPrice] = useState(course.strikethrough_price > 0);
    ...
    function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        modules.forEach((mod, modIdx) => {
            mod.lessons?.forEach((lesson: Lesson, lessonIdx: number) => {
        ...
        router.post(route('courses.update', { course: course.id }), formData, { forceFormData: true });
    }
    ...
    return (
        <AdminLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="px-4 py-4 md:px-6">
                <h1 className="text-2xl font-semibold">Edit Kelas</h1>
                <p className="text-muted-foreground mb-6 max-w-2xl text-sm">
                    Ubah data kelas di bawah ini. Setelah selesai, klik tombol "Simpan Perubahan".
                </p>
                                            ...
                                            <Popover open={isItemPopoverOpen} onOpenChange={setIsItemPopoverOpen}>
                                                <PopoverTrigger asChild>
                                                    <FormControl>
                                                        ...
                                                        >
                                                            {field.value
                                                                ? categories.find((category: Category) => category.id === field.value)?.name
                                                                : 'Pilih kategori'}
