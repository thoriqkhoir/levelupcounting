...
export default function EditBootcamp({
    ...
    const [showStrikethroughPrice, setShowStrikethroughPrice] = useState(bootcamp.strikethrough_price > 0);
    ...
    const form = useForm<z.infer<typeof formSchema>>({
        ...
        defaultValues: {
            mentor_ids: (bootcamp.mentors ?? []).map((m) => m.id),
            title: bootcamp.title ?? '',
    ...
    function toLocalYmd(value?: string) {
        ...
        // If value is an ISO datetime (e.g. ends with Z / has timezone), convert to local date.
        if (value.includes('T')) {
