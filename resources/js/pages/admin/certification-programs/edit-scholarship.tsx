...
import { CalendarFold, Check, ChevronsUpDown, GraduationCap } from 'lucide-react';
...
const formSchema = z
    ...
    .refine(
        (data) => {
            if (data.strikethrough_price > 0) return data.strikethrough_price > data.scholarship_price;
            return true;
        },
        { message: 'Harga coret harus lebih besar dari harga beasiswa.', path: ['strikethrough_price'] },
    );
...
function DeadlineField({
    ...
    name: 'registration_deadline' | 'socialization_registration_deadline';
    label: string;
                    ...
                    <div className="flex flex-wrap gap-2">
                        <Popover open={open} onOpenChange={setOpen}>
                            <PopoverTrigger asChild>
                                <FormControl>
                                    <Button variant="outline" type="button" className="w-36 justify-between font-normal">
                                        {dateVal
                                            ? dateVal.toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })
                                            : 'Pilih tanggal'}
...
export default function EditScholarshipCertificationProgram({ program, categories, mentors }: EditScholarshipProps) {
    ...
    const [socializationScheduleItems, setSocializationScheduleItems] = useState<BootcampSchedule[]>(
        socializationSchedules.map((s) => ({
            id: s.id,
            title: s.title,
            schedule_date: s.schedule_date,
            day: s.day,
    ...
    const form = useForm<FormValues>({
        ...
        defaultValues: {
            ...
            price: program.price,
            scholarship_price: program.scholarship_price ?? 0,
            scholarship_flow: program.scholarship_flow ?? '',
            registration_deadline: program.registration_deadline ?? '',
            socialization_registration_deadline: program.socialization_registration_deadline ?? '',
    ...
    function onSubmit(values: FormValues) {
        const submitData = { ...values, type: 'scholarship', schedules, socialization_schedules: socializationScheduleItems, _method: 'PUT' };
    ...
    return (
        <AdminLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit Program Beasiswa - ${program.title}`} />
            <div className="px-4 py-4 md:px-6">
                <h1 className="text-2xl font-semibold">Edit Program Sertifikasi Beasiswa</h1>
                <p className="text-muted-foreground mb-6 max-w-2xl text-sm">
                    Edit detail program sertifikasi beasiswa. Perubahan akan disimpan ke data yang sudah ada.
                </p>
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 items-start gap-6 lg:grid-cols-2">
                        ...
                        <div className="space-y-6 rounded-lg border p-4">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="font-medium">Informasi Program Beasiswa</h3>
                            </div>
                                    ...
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Editor
                                                ...
                                                init={{ placeholder: 'Masukkan deskripsi lengkap program...', ...tinyInit }}
                                            />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                    ...
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Editor
                                                ...
                                                value={field.value ?? ''}
                                                onEditorChange={(c) => field.onChange(c)}
                                                init={{ placeholder: 'Masukkan alur/tahapan seleksi beasiswa...', ...tinyInit }}
                                            />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="rounded-lg border p-4">
                                <h3 className="mb-4 font-medium">Harga Beasiswa</h3>
                                <div className="space-y-4">
