...
import { AlertTriangle, BookMarked, CalendarFold, Check, ChevronsUpDown, GraduationCap } from 'lucide-react';
                                                        ...
                                                        <CommandItem
                                                            ...
                                                            onSelect={() => {
                                                                try {
                                                                    ...
                                                                    let mappedSocialization: BootcampSchedule[] = rawSchedules
                                                                        .filter((s: any) => s && (s.schedule_type === 'socialization' || s.schedule_type === 'sosialisasi'))
                                                                        .map((s: any) => {
                                                                            const scheduleDate = extractDate(s.schedule_date || s.date || s.event_date || s.start_date || s.start_time);
                                                                            const day = s.day ? String(s.day).toLowerCase() : getDayFromDateStr(scheduleDate);
                                                                            const startTime = extractTime(s.start_time || s.time_start || s.jam_mulai);
                                                                            ...
                                                                            return {
                                                                                ...
                                                                                start_time: startTime,
                                                                                end_time: endTime,
                                                                                title: s.title || s.name || s.session_title || 'Sosialisasi'
                                                                            };
                                                                        })
                                                                        .filter((s: BootcampSchedule) => Boolean(s.schedule_date));
