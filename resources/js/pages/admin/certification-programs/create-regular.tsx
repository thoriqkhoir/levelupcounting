...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Tambah Program Sertifikasi Reguler',
        href: route('certification-programs.create', { type: 'regular' }),
    },
];
...
export default function CreateRegularCertificationProgram({ categories, mentors }: CreateRegularProps) {
    ...
    useEffect(() => {
        fetch(route('admin.biinsight-import.programs') + '?type=certification_program')
            ...
            .then((res) => {
                if (res.success) {
                    // Filter regular only
                    const regularPrograms = (res.data || []).filter((p: any) => p.certif_type === 'regular');
                    setBiinspiraPrograms(regularPrograms);
                }
            })
    ...
    function onSubmit(values: FormValues) {
        const submitData = { ...values, type: 'regular', schedules };
    ...
    return (
        <AdminLayout breadcrumbs={breadcrumbs}>
            <Head title="Tambah Program Sertifikasi Reguler" />
            <div className="px-4 py-4 md:px-6">
                <h1 className="text-2xl font-semibold">Tambah Program Sertifikasi Reguler</h1>
                <p className="text-muted-foreground mb-6 max-w-2xl text-sm">
                    Silakan isi form di bawah ini untuk membuat program sertifikasi reguler. Anda dapat mengatur biaya, mentor, dan informasi
                    pendaftaran.
                </p>
