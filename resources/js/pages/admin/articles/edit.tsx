...
const breadcrumbs = (article: Article): BreadcrumbItem[] => [
    {
        title: 'Artikel',
...
export default function EditArticle({ article, categories }: EditProps) {
    ...
    function onSubmit(values: FormValues) {
        ...
        router.post(route('articles.update', article.id), formData, { forceFormData: true });
    }

    return (
        <AdminLayout breadcrumbs={breadcrumbs(article)}>
            <Head title={`Edit Artikel - ${article.title}`} />
            <div className="px-4 py-4 md:px-6">
                <h1 className="text-2xl font-semibold">Edit {article.title}</h1>
                <p className="text-muted-foreground mb-6 max-w-2xl text-sm">
                    Edit informasi artikel. Perubahan akan meng-update slug dan URL jika judul berubah.
                </p>
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 items-start gap-6 lg:grid-cols-2">
                        ...
                        <div className="space-y-6 rounded-lg border p-4">
                            ...
                            <div className="rounded-md border border-gray-200 bg-gray-50 p-4">
                                <h4 className="mb-2 font-medium text-gray-900">Status Artikel</h4>
                                <div className="space-y-2 text-sm text-gray-700">
                                    <div className="text-muted-foreground text-xs">
                                        Ubah status artikel di halaman detail setelah menyimpan perubahan.
                                    </div>
                                </div>
                            </div>
                        </div>
