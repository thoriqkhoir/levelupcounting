...
import { ArrowRight, Award, Calendar, Clock, GalleryVerticalEnd, Package, Percent, Search } from 'lucide-react';
...
export default function PartnershipProductSection({ categories, partnershipProducts }: PartnershipProductProps) {
    ...
    const filteredProducts = partnershipProducts.filter((p) => {
        const matchSearch = p.title.toLowerCase().includes(search.toLowerCase());
        const matchCategory = selectedCategory === null ? true : p.category.id === selectedCategory;
    ...
    return (
        <section className="mx-auto w-full max-w-7xl px-4 pb-24 pt-10" id="partnership-products">
            ...
            <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                {/* Kategori */}
                <div className="flex-1 overflow-x-auto hide-scrollbar" ref={categoryRef} onMouseDown={handleMouseDown} onMouseLeave={handleMouseLeave} onMouseUp={handleMouseUp} onMouseMove={handleMouseMove} style={{ cursor: isDragging.current ? 'grabbing' : 'grab' }}>
                ...
                <div className="relative w-full md:w-72 lg:w-96 shrink-0">
                    ...
                    <Input type="search" placeholder="Cari sertifikasi..." value={search} onChange={(e) => setSearch(e.target.value)} className="h-12 w-full rounded-full border-2 border-border bg-background pl-11 pr-4 shadow-sm transition-colors focus-visible:border-primary focus-visible:ring-0" />
                </div>
            </div>
            ...
            <div className="mb-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {visibleProducts.length === 0 ? (
                    ...
                    visibleProducts.map((product) => {
                        ...
                        const discount = calcDiscount(product.strikethrough_price, product.price);

                        return (
                            <Link key={product.id} href={route('partnership-product.detail', product.slug)} className="group h-full">
                                <div className="relative flex h-full flex-col overflow-hidden rounded-2xl border-2 border-gray-100 bg-white shadow-sm transition-all duration-300 group-hover:-translate-y-2 group-hover:shadow-xl group-hover:border-primary group-hover:ring-2 group-hover:ring-primary dark:border-zinc-700 dark:bg-zinc-800">
                                    ...
                                    <div className="relative w-full overflow-hidden">
                                        ...
                                        {/* Discount / Urgency / Free Badge - Top Right */}
                                        {product.price === 0 ? (
                                        ...
                                        <div className="absolute right-2 bottom-2 z-20 rounded-lg border border-white/40 bg-white/30 px-2 py-1 shadow backdrop-blur-md dark:bg-gray-800/30">
                                            <div className="flex items-center gap-2">
                                                <Package size={12} />
                                                <p className="text-xs font-semibold text-black dark:text-gray-400">{product.duration_days} Hari</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-1 flex-col gap-3 p-5">
                                        <h2 className="line-clamp-2 text-lg lg:text-xl font-bold text-gray-900 dark:text-gray-100">{product.title}</h2>
                                        {product.short_description && (
                                            <p className="line-clamp-2 text-sm text-muted-foreground leading-relaxed">{product.short_description}</p>
                                        )}
                                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                            <Calendar className="h-4 w-4" />
                                            <span>Batas: {format(deadlineDate, 'dd MMM yyyy', { locale: id })}</span>
                                        </div>
                                        <div>
                                            {product.strikethrough_price > 0 && product.strikethrough_price > product.price && (
                                                <p className="text-base text-red-500 line-through">Rp. {product.strikethrough_price.toLocaleString('id-ID')}</p>
                                            )}
                                            {product.price === 0 ? (
                                                <p className="text-2xl font-bold text-green-600 dark:text-green-400">Gratis</p>
                                            ) : (
                                                <p className="text-2xl font-bold text-green-600 dark:text-green-400">Rp. {product.price.toLocaleString('id-ID')}</p>
                                            )}
                                        </div>
                                    </div>
