...
export default function KeyPointsSection({ partnershipProduct }: { partnershipProduct: PartnershipProduct }) {
    if (!partnershipProduct.description && !partnershipProduct.key_points) {
        return null;
    }

    // Parse key points if it's HTML list
    const parseKeyPoints = (keyPointsHtml: string) => {
        const parser = new DOMParser();
        const doc = parser.parseFromString(keyPointsHtml, 'text/html');
        const listItems = doc.querySelectorAll('li');
        return Array.from(listItems).map((li) => li.textContent || '');
    };

    const keyPointsList = partnershipProduct.key_points ? parseKeyPoints(partnershipProduct.key_points) : [];
    ...
    return (
        <section className="relative mx-auto w-full max-w-7xl px-4 py-12">
                        ...
                        <div className="group relative h-full overflow-hidden rounded-3xl border-2 border-blue-200 bg-gradient-to-br from-blue-50/80 via-white to-blue-100/50 p-8 shadow-xl transition-all duration-300 hover:shadow-2xl dark:border-blue-800 dark:from-blue-950/30 dark:via-gray-900 dark:to-blue-900/20">
                            ...
                            <div className="mb-6 flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600 dark:text-gray-400">Informasi lengkap program ini</p>
                                </div>
                            </div>
                    ...
                    >
                        <div className="group relative h-full overflow-hidden rounded-3xl border-2 border-green-200 bg-gradient-to-br from-green-50/80 via-white to-green-100/50 p-8 shadow-xl transition-all duration-300 hover:shadow-2xl dark:border-green-800 dark:from-green-950/30 dark:via-gray-900 dark:to-green-900/20">
                            ...
                            <div className="mb-6 flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Yang Anda Dapatkan</h3>
                                    <p className="text-sm text-gray-600 dark:text-gray-400">Manfaat program ini</p>
                                </div>
                            </div>
                            ...
                            {keyPointsList.length > 0 ? (
                                ...
                                <div
                                    ...
                                    dangerouslySetInnerHTML={{ __html: partnershipProduct.key_points }}
                                />
                            )}
            ...
            {(partnershipProduct.description && !partnershipProduct.key_points) ||
            (!partnershipProduct.description && partnershipProduct.key_points) ? (
                <motion.div
                ...
                >
                    <div className="relative overflow-hidden rounded-3xl border-2 border-primary/20 bg-gradient-to-br from-primary/5 via-white to-secondary/5 p-8 shadow-xl dark:from-primary/10 dark:via-gray-900 dark:to-secondary/10">
                        <div className="mb-6 flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-2xl font-bold text-gray-900 dark:text-white">
                                    {partnershipProduct.description ? 'Tentang Program' : 'Yang Anda Dapatkan'}
                                </h3>
                                <p className="text-sm text-gray-600 dark:text-gray-400">
                                    {partnershipProduct.description ? 'Informasi lengkap program ini' : 'Manfaat program ini'}
                                </p>
                            </div>
                        </div>
                        ...
                        {partnershipProduct.key_points && keyPointsList.length > 0 && (
                            <div className="grid gap-3 sm:grid-cols-2">
                                {keyPointsList.map((point, index) => (
                                    ...
                                    >
                                        ...
                                        <p className="flex-1 text-sm font-medium text-gray-700 dark:text-gray-300">{point}</p>
                                    </motion.div>
                                ))}
                            </div>
                        )}
